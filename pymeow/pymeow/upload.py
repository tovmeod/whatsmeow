...
class Client:
    """
    Partial Client class showing upload integration.

    In the actual implementation, this would be mixed into the main Client class.
    """
    ...
    async def refresh_media_conn(self, ctx, force=False):
        """Refresh media connection info."""
...
class Uploader:
    ...
    async def upload(self, ctx, plaintext: bytes, app_info: MediaType) -> UploadResponse:
        """
        ...
        For example, to send an image:
            ...
            resp = await client.upload(ctx, your_image_bytes, MediaType.IMAGE)
            # handle error
            ...
            _, err = await client.send_message(ctx, target_jid, waE2E_pb2.Message(imageMessage=image_msg))
            # handle error again
        ...
        # Get media keys
        iv, cipher_key, mac_key, _ = self._get_media_keys(resp.media_key, app_info)
        ...
        # Upload the data
        await self._raw_upload(ctx, io.BytesIO(data_to_upload), len(data_to_upload),
                              resp.file_enc_sha256, app_info, False, resp)

        return resp

    async def upload_reader(self, ctx, plaintext: BinaryIO, temp_file: Optional[BinaryIO],
        ...
        try:
            ...
            resp.file_sha256, resp.file_enc_sha256, resp.file_length, upload_size = encrypt_stream(
                cipher_key, iv, mac_key, plaintext, temp_file
            )
            ...
            # Upload the data
            await self._raw_upload(ctx, temp_file, upload_size, resp.file_enc_sha256,
                                  app_info, False, resp)
    ...
    async def upload_newsletter(self, ctx, data: bytes, app_info: MediaType) -> UploadResponse:
        ...
        * There's a "media handle" that needs to be passed in SendRequestExtra.
        ...
        Example:
            ...
            resp = await client.upload_newsletter(ctx, your_image_bytes, MediaType.IMAGE)
            # handle error
            ...
            _, err = await client.send_message(ctx, newsletter_jid, waE2E_pb2.Message(imageMessage=image_msg),
        ...
        # Upload the data
        await self._raw_upload(ctx, io.BytesIO(data), resp.file_length,
                              resp.file_sha256, app_info, True, resp)

        return resp

    async def upload_newsletter_reader(self, ctx, data: BinaryIO, app_info: MediaType) -> UploadResponse:
        """
        Upload the given attachment to WhatsApp servers without encrypting it first.
        ...
        # Upload the data
        await self._raw_upload(ctx, data, resp.file_length,
                              resp.file_sha256, app_info, True, resp)

        return resp

    async def _raw_upload(self, ctx, data_to_upload: BinaryIO, upload_size: int,
        ...
        try:
            media_conn = await self.client.refresh_media_conn(ctx, False)
        ...
        if self.client.messenger_config is not None:
        ...
        # Select host
        host = None
        if self.client.messenger_config is not None and media_conn.hosts:
        ...
        await self.client.ensure_http_session()
        ...
        try:
            # Make the request
            async with self.client.http_session.post(upload_url, data=data_to_upload, headers=headers) as response:
    ...
    def _get_media_keys(self, media_key: bytes, app_info: MediaType) -> Tuple[bytes, bytes, bytes, bytes]:
        """
