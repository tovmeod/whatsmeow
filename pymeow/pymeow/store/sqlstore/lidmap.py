...
async def fill_cache(self, ctx: Any) -> None:
    """
    ...
    async with self.lid_cache_lock:
        rows = await self.db.fetch_all(ctx, GET_ALL_LID_MAPPINGS_QUERY)
...
async def _get_lid_mapping(self, ctx: Any, source: JID, target_server: str, query: str,
    ...
    async with self.lid_cache_lock:
        row = await self.db.fetch_one(ctx, query, source.user)
...
async def get_lid_for_pn(self, ctx: Any, pn: JID) -> Tuple[JID, Optional[Exception]]:
    """
    ...
    try:
        jid = await self._get_lid_mapping(
            ctx, pn, "lid.whatsapp.net", GET_LID_FOR_PN_QUERY,
...
async def get_pn_for_lid(self, ctx: Any, lid: JID) -> Tuple[JID, Optional[Exception]]:
    """
    ...
    try:
        jid = await self._get_lid_mapping(
            ctx, lid, "s.whatsapp.net", GET_PN_FOR_LID_QUERY,
...
async def _unlocked_put_lid_mapping(self, ctx: Any, lid: JID, pn: JID) -> None:
    """
    ...
    await self.db.execute(ctx, DELETE_EXISTING_LID_MAPPING_QUERY, lid.user, pn.user)

    # Insert the new mapping
    await self.db.execute(ctx, PUT_LID_MAPPING_QUERY, lid.user, pn.user)

    # Update cache
    self.pn_to_lid_cache[pn.user] = lid.user
    self.lid_to_pn_cache[lid.user] = pn.user

async def put_lid_mapping(self, ctx: Any, lid: JID, pn: JID) -> None:
    """
    Store a LID mapping.
    ...
    async with self.lid_cache_lock:
        ...
        async def _do_mapping(ctx: Any) -> None:
            await self._unlocked_put_lid_mapping(ctx, lid, pn)

        await self.db.transaction(ctx, _do_mapping)

async def put_many_lid_mappings(self, ctx: Any, mappings: List[store.LIDMapping]) -> None:
    """
    ...
    async with self.lid_cache_lock:
        ...
        # Store in database within a transaction
        async def _do_mappings(ctx: Any) -> None:
            for mapping in valid_mappings:
                await self._unlocked_put_lid_mapping(ctx, mapping.lid, mapping.pn)

        await self.db.transaction(ctx, _do_mappings)

# Add method implementations to the CachedLIDMap class
CachedLIDMap.fill_cache = fill_cache
CachedLIDMap._get_lid_mapping = _get_lid_mapping
CachedLIDMap.get_lid_for_pn = get_lid_for_pn
CachedLIDMap.get_pn_for_lid = get_pn_for_lid
CachedLIDMap._unlocked_put_lid_mapping = _unlocked_put_lid_mapping
CachedLIDMap.put_lid_mapping = put_lid_mapping
CachedLIDMap.put_many_lid_mappings = put_many_lid_mappings
