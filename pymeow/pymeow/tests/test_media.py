...
from pymeow.download import MediaType
    ...
    async def test_send_document_with_progress_callback(self):
        ...
        with (
            ...
            await client.send_document(
                to_jid_str, doc_path, file_name=custom_file_name, progress_callback=progress_callback_mock
            )

            mock_mimetype.assert_called_once_with(custom_file_name)
            mock_file.assert_called_once_with(doc_path, "rb")
            mock_upload.assert_called_once_with(
                doc_path, MediaType.DOCUMENT, progress_callback=progress_callback_mock, file_name=custom_file_name
            )
