...
from pymeow.pymeow.client import Client # Corrected import
...
async def test_send_reply_success_text_message():
    ...
    mock_get_quoted_elements = AsyncMock(
        return_value=(mock_stanza_id, mock_participant_jid, mock_quoted_message_proto)
    )
...
async def test_send_contacts_success_multiple_contacts():
    ...
    contacts_list = [
        {"contact_jid": "user1@s.whatsapp.net", "name": "User One", "phone_number": "+111222333"},
        {"contact_jid": "user2@other.domain", "name": "User Two", "phone_number": "444555666"} # Non-WA JID, number only
    ]
