...
from pymeow.pymeow.exceptions import PymeowError, PymeowTimeoutError
from pymeow.pymeow.datatypes.jid import JID
from pymeow.pymeow.binary.node import Node # For constructing mock response nodes
...
class TestGroupManagement:
    ...
    async def test_create_group_success(self):
        ...
        mock_resp_group_node = Node("group", {
            "id": mock_group_id,
        ...
        mock_success_node = Node("iq", {"type": "result", "id": "some_iq_id", "to": str(my_jid)}, [mock_resp_group_node])
    ...
    async def test_update_group_participants_no_participants_node_in_response(self):
        ...
        mock_empty_success_node = Node("iq", {"type": "result", "from": JID.from_string(group_jid_str).to_string(), "id": "update_empty_ack"})
    ...
    async def test_update_group_participants_malformed_participant_in_response(self):
        ...
        group_jid_str = "testgroup@g.us"
        actions = [{"action": "add", "jid": "add_me@s.whatsapp.net"}, {"action": "remove", "jid": "remove_me@s.whatsapp.net"}]
        ...
        mock_malformed_response_node = Node("iq", {"type": "result", "from": JID.from_string(group_jid_str).to_string(), "id": "update_malformed_ack"}, [mock_resp_participants_node])
    ...
    async def test_update_group_participants_success_all_actions(self):
        ...
        mock_resp_participants_children = [
            ...
            Node("promote", {}, [Node("participant", {"jid": "promote@s.whatsapp.net", "code": "200", "admin": "true"})]),
            Node("demote", {}, [Node("participant", {"jid": "demote@s.whatsapp.net", "code": "200", "admin": "false"})]),
        ]
        mock_resp_participants_node = Node("participants", {}, mock_resp_participants_children)
        mock_success_node = Node("iq", {"type": "result", "from": str(group_jid_obj), "id": "update_ack"}, [mock_resp_participants_node])
        mock_send_node_iq.return_value = mock_success_node
        ...
        expected_results = [
            {"jid": JID.from_string("add@s.whatsapp.net"), "action": "add", "status_code": 200, "error_text": None, "admin": None},
            {"jid": JID.from_string("remove@s.whatsapp.net"), "action": "remove", "status_code": 200, "error_text": None, "admin": None},
            {"jid": JID.from_string("promote@s.whatsapp.net"), "action": "promote", "status_code": 200, "error_text": None, "admin": "true"},
            {"jid": JID.from_string("demote@s.whatsapp.net"), "action": "demote", "status_code": 200, "error_text": None, "admin": "false"},
        ]
        for i, res in enumerate(results):
            ...
            assert res.get("admin") == expected_results[i]["admin"] # Use .get for admin as it might not always be present
    ...
    async def test_update_group_participants_partial_success_mixed_codes(self):
        ...
        mock_resp_participants_children = [
            Node("add", {}, [
                ...
                Node("participant", {"jid": "add_fail_403@s.whatsapp.net", "code": "403"}, [Node("error", {"text": "Not allowed"})]),
            ]),
            Node("promote", {}, [
                Node("participant", {"jid": "promote_ok@s.whatsapp.net", "code": "200", "admin": "true"}),
                Node("participant", {"jid": "promote_fail_404@s.whatsapp.net", "code": "404"}, [Node("error", {"text": "Not found"})]),
            ]),
        ]
        mock_resp_participants_node = Node("participants", {}, mock_resp_participants_children)
        mock_success_node = Node("iq", {"type": "result", "from": str(group_jid_obj), "id": "update_ack_partial"}, [mock_resp_participants_node])
        mock_send_node_iq.return_value = mock_success_node
        ...
        expected_results_data = [
            ...
            {"jid_str": "add_fail_403@s.whatsapp.net", "action": "add", "code": 403, "error": "Not allowed", "admin": None},
            {"jid_str": "promote_ok@s.whatsapp.net", "action": "promote", "code": 200, "error": None, "admin": "true"},
            {"jid_str": "promote_fail_404@s.whatsapp.net", "action": "promote", "code": 404, "error": "Not found", "admin": None}, # promote fail, admin might be absent or false
        ]
    ...
    async def test_update_group_participants_iq_error(self):
        ...
        mock_iq_error_node = Node("iq", {"type": "error", "from": JID.from_string(group_jid_str).to_string(), "id": "update_err_ack"}, [mock_error_content_node])
    ...
    async def test_update_group_participants_timeout(self):
        ...
        mock_send_node_iq = AsyncMock(side_effect=PymeowTimeoutError("Timeout updating participants"))
        client._send_node_with_iq_response = mock_send_node_iq

        group_jid_str = "testgroup@g.us"
        actions = [{"action": "add", "jid": "add_me_timeout@s.whatsapp.net"}]

        with pytest.raises(PymeowTimeoutError, match="Timeout updating participants"):
            await client.update_group_participants(group_jid_str, actions)
    ...
    async def test_set_group_description_success(self):
        ...
        mock_success_response_node = Node("iq", {
            "type": "result",
            "id": "set_desc_ack",
            "from": str(group_jid_obj)
        }, [mock_resp_desc_node])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("406", "not-acceptable", r"Cannot set group subject: Not acceptable \(406\)"), # e.g. group not found by that JID
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("404", "item-not-found", r"Cannot leave group: Group not found \(404\)"), # 'item-not-found' is common for 404
    ...
    async def test_join_group_success(self):
        ...
        }, [Node("participant", {"jid": mock_owner_jid_str, "type": "admin"})]) # Add a sample participant

        mock_success_node = Node("iq", {"type": "result", "id": "join_iq_id", "to": str(my_jid)}, [mock_resp_group_node])
        mock_send_node_iq.return_value = mock_success_node
    ...
    async def test_get_group_invite_link_no_invite_node_in_response(self):
        ...
        with pytest.raises(PymeowError, match="No invite code in response"): # Or "No invite node..." depending on exact error
    ...
    async def test_get_group_invite_link_success(self):
        ...
        mock_success_node = Node("iq", {"type": "result", "id": "some_iq_id", "to": str(client.jid)}, [mock_resp_invite_node])
        mock_send_node_iq.return_value = mock_success_node
    ...
    async def test_get_group_invite_link_reset_success(self):
        ...
        mock_success_node_reset = Node("iq", {"type": "result"}, [mock_resp_invite_node_reset]) # Corrected: Removed client.jid from 'to'
    ...
    async def test_get_group_invite_link_api_error(self):
        ...
        mock_error_content_node = Node("error", {"code": "403", "text": "Not allowed"})
        mock_error_response_node = Node("iq", {"type": "error"}, [mock_error_content_node]) # Removed id and to for simplicity in mock
        mock_send_node_iq.return_value = mock_error_response_node
    ...
    async def test_create_group_only_self_as_participant(self):
        ...
        mock_resp_group_node = Node("group", {
            "id": mock_group_id, "creator": mock_creator_jid_str,
            "creation": mock_creation_time, "subject": subject,
            "s_t": "st", "s_o": mock_creator_jid_str
        })
